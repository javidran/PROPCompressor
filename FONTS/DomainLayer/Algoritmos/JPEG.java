    ...
    public OutputAlgoritmo comprimir(byte[] datosInput) throws FormatoErroneoException {
        ...
        for (int x = 0; x < downSampledPaddedHeight; x += 8) { //image DCT-II and quantization (done in pixel squares of 8x8) for chrominance
            ...
            IntStream.range(0, downSampledPaddedWidth).parallel().filter(y -> y % 8 == 0).forEach(y -> { //for each chrominance pixel square of 8x8 of the image, DCT-II algorithm is applied, letting calculate the image frequencies
                ...
                tempResultCbCr[finalX /8][y/8] = new ArrayList<>();
                ...
                tempResultCbCr[finalX /8][y/8].add((byte)sizeOfBlock); //size in bits of block defined before reading each block in order to know how many bytes have to be read
                tempResultCbCr[finalX /8][y/8].add((byte)offsetOfBlock);
                //addition of block to result
                tempResultCbCr[finalX /8][y/8].addAll(toByteList(rleCb)); //dumping the Huffman block into result
                ...
                tempResultCbCr[finalX /8][y/8].add((byte)offsetOfBlock);
                //addition of block to result
                tempResultCbCr[finalX /8][y/8].addAll(toByteList(rleCr)); //dumping the Huffman block into result
            });
        }
